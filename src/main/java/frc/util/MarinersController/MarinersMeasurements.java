...
/**
 * A class to measure the position, velocity, and acceleration of a system used in conjunction with
 * the {@link MarinersController} class can calculate the velocity and acceleration from the
 * position or can use the built-in measurements of the motor controller
 */
public class MarinersMeasurements {
  /** the supplier for the position of the system */
  private Supplier<Double> position;

  /** the supplier for the velocity of the system (can be calculated from the position) */
  private Supplier<Double> velocity;

  /** the supplier for the acceleration of the system (can be calculated from the velocity) */
  private Supplier<Double> acceleration;

  /**
   * the current position of the system (after gear ratio) update when {@link #update(double)} is
   * called
   */
  private double currentPosition;

  /**
   * the current velocity of the system (after gear ratio) update when {@link #update(double)} is
   * called
   */
  private double currentVelocity;

  /**
   * the current acceleration of the system (after gear ratio) update when {@link #update(double)}
   * is called
   */
  private double currentAcceleration;

  /** the gear ratio of the system each measurement is divided by this value */
  ...
  /**
   * update the measurements of the system NEEDS TO BE CALLED PERIODICALLY
   *
   ...
   * @param gearRatio the value the original measurements are divided by
   */
  public MarinersMeasurements(
      Supplier<Double> position,
      Supplier<Double> velocity,
      Supplier<Double> acceleration,
      double gearRatio) {
    this.gearRatio = gearRatio;
   ...
   */
  public MarinersMeasurements(
      Supplier<Double> position, Supplier<Double> velocity, double gearRatio) {
    this.gearRatio = gearRatio;
