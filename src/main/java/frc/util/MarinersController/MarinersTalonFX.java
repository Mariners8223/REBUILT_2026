...
public class MarinersTalonFX extends MarinersController {
  ...
  /**
   * the configuration for the motor output (needed that info is not lost when changing the motor
   * output)
   */
  ...
  /**
   * create a new measurement object for the motor using the built-in position, velocity, and
   * acceleration this creates a measurement object that waits for all the signals to update before
   * returning the value (that means that it is a blocking call)
   *
  ...
  private MarinersMeasurements createMeasurement(double gearRatio) {
    return new MarinersMeasurementsCTRE(
        motor.getPosition(), motor.getVelocity(), motor.getAcceleration(), gearRatio);
  }
   ...
   * @param gearRatio the gear ratio of the motor
   */
  public MarinersTalonFX(
      String name, ControllerLocation location, int id, PIDFGains gains, double gearRatio) {
