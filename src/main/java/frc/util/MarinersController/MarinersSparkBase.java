...
/**
 * a class for spark motor controllers (for both spark max and spark flex) this class is used for
 * controlling spark motor controllers this class is a subclass of {@link MarinersController} this
 * does automatic error reporting to the driver station and also built in logging in advantage kit
...
public class MarinersSparkBase extends MarinersController {
  ...
  /** the configuration for the motor controller used for setting the motor controller's settings */
  private final SparkBaseConfig config;

  /** the type of motor controller used for when setting an external encoder */
   ...
   *
   * @param gearRatio the gear ratio of the motor controller (the value that the measurements will
   *     be divided by)
   */
  private void setMeasurements(double gearRatio) {
    ...
    super.setMeasurements(
        new MarinersMeasurements(
            encoder::getPosition, () -> encoder.getVelocity() / 60, gearRatio));
  }
  ...
  private void configSparkBase(SparkBase sparkBase) {
    ...
    signalsConfig
        .appliedOutputPeriodMs(APPLIED_OUTPUT_PERIOD_MS)
  ...
  /**
   * creates a new spark motor controller this will create a motor that can only be controlled by
   * duty cycle or voltage until {@link #setPIDF(PIDFGains)} or {@link #setPIDF(PIDFGains,
   * Function)} is called
   *
   ...
   */
  public MarinersSparkBase(
      String name, ControllerLocation location, int id, boolean isBrushless, MotorType type) {
    super(name, location);
    ...
    motor =
        switch (type) {
          case SPARK_MAX -> new SparkMax(id, motorType);
          case SPARK_FLEX -> new SparkFlex(id, motorType);
        };
   ...
   * @param gearRatio the gear ratio of the motor controller (the value that the measurements will
   *     be divided by)
   * @param firstDerivativeLimit the first derivative limit for the motor controller (used for
   *     motion profiling)
   * @param secondDerivativeLimit the second derivative limit for the motor controller (used for
   *     motion profiling)
   */
  public MarinersSparkBase(
      String name,
      ...
      PIDFGains gains,
      double gearRatio,
      double firstDerivativeLimit,
      double secondDerivativeLimit) {
    this(name, location, id, isBrushless, type);
  ...
  /**
   * if using a thorough bore encoder use this function instead of {@link
   * #useExternalAbsoluteEncoder(boolean, double, double)} this will have more precise measurements
   * and better overall use this only if you have a thorough bore encoder connected to a spark flex
   *
   * @param inverted if the encoder is inverted (use to set the same direction as the motor)
   * @param zeroOffset the zero offset for the encoder (a value between 0 and 1) that will be
   *     removed from the absolute encoder's position
   * @param gearRatio the gear ratio of the motor controller (the value that the measurements will
   *     be divided by) (need to add this if using a gear ratio between the encoder and the motor)
   */
  public void useExternalAbsoluteEncoderUsingRelative(
      boolean inverted, double zeroOffset, double gearRatio) {
    ...
    config
        .signals
        .externalOrAltEncoderPosition((int) (1000 / RUN_HZ))
  ...
  /**
   * if using a thorough bore encoder use this function instead of {@link
   * #useExternalAbsoluteEncoder(boolean, double)} this will have more precise measurements and
   * better overall use this only if you have a thorough bore encoder connected to a spark flex
   *
   * @param inverted if the encoder is inverted (use to set the same direction as the motor)
   * @param zeroOffset the zero offset for the encoder (a value between 0 and 1) that will be
   *     removed from the absolute encoder's position
  ...
  /**
   * sets the measurements for the motor controller using an external encoder also works for using
   * the motor's built-in controller if using a spark flex controller and a thorough bore encoder
   * use {@link #useExternalAbsoluteEncoderUsingRelative(boolean, double, double)}
   *
   * @param inverted if the encoder is inverted (use to set the same direction as the motor)
   * @param zeroOffset the zero offset for the encoder (a value between 0 and 1) that will be
   *     removed from the absolute encoder's position
  ...
  public void useExternalAbsoluteEncoder(boolean inverted, double zeroOffset, double gearRatio) {
    ...
    super.setMeasurements(new MarinersMeasurements(encoder::getPosition, gearRatio));
  }

  /**
   * sets the measurements for the motor controller using an external encoder also works for using
   * the motor's built-in controller if using a spark flex controller and a thorough bore encoder
   * use {@link #useExternalAbsoluteEncoderUsingRelative(boolean, double)}
   *
   * @param inverted if the encoder is inverted (use to set the same direction as the motor)
   * @param zeroOffset the zero offset for the encoder (a value between 0 and 1) that will be
   *     removed from the absolute encoder's position
  ...
  private AbsoluteEncoder getAbsoluteEncoder(
    ...
    config
        .signals
        .absoluteEncoderPositionPeriodMs((int) (1000 / RUN_HZ))
   ...
   * @param thresholdRPM the rpm that below it the motor will be considered stalled (after that
   *     value the current limit will scale until the limit set until free speed)
   * @param thresholdCurrentLimit the current threshold for the motor (above this value the motor
   ...
   * sets the current limits for the motor
   *
   * @param currentLimit the current limit for the motor (the motor will try to stay below this
   *     value)
   * @param currentThreshold the current threshold for the motor (above this value the motor will be
  ...
  protected void updateInputs(MotorInputs inputs) {
    ...
    inputs.currentDraw =
        ((inputs.currentOutput * inputs.voltageInput + ENERGY_LOSS_WATTS) / inputs.voltageInput);
  ...
  protected void setMotorDeadBandDutyCycleMotor(double deadBand) {
    reportWarning(
        "Dead band for spark controllers is only available for controllers running on rio");
  }
  ...
  protected void setOutput(double output, ControlMode controlMode, double feedForward) {
    ...
    // REVLibError error = motor.getClosedLoopController().setReference(output, controlType, 0,
    // feedForward);
    REVLibError error =
        motor
            .getClosedLoopController()
            .setReference(output, controlType, ClosedLoopSlot.kSlot0, feedForward);
