...
/**
 * A class to control a simulated motor used in conjunction with the {@link MarinersController}
 * class can be used to simulate any motor
 */
public class MarinersSimMotor extends MarinersController {
  ...
  /**
   * the voltage of the master motor (if this motor is a follower) (null if this motor is not a
   * follower)
   */
   ...
   * @param motorReduction the ratio between motor and the mechanism (motor turns / mechanism turns)
   *     (no unit conversion)
   * @param unitConversion if using any unit conversion (for example, if the mechanism is in meters,
   *     the unit conversion would be the meters per rotation)
   * @param momentOfInertia the moment of inertia of the system (in kg m^2) (after motor reduction
   *     (in mechanism units))
   */
  public MarinersSimMotor(
      String name,
      DCMotor motorType,
    ...
    motor =
        new DCMotorSim(
            LinearSystemId.createDCMotorSystem(motorType, momentOfInertia, motorReduction),
  ...
  /**
   * creates the controller using a gear ratio of 1
   *
   * @param name the name of the controller (for logging)
   * @param motorType the type of motor to simulate (do not include the reduction)
   * @param motorReduction the ratio between motor and the mechanism (motor turns / mechanism turns)
   ...
   */
  public MarinersSimMotor(
      String name, DCMotor motorType, double motorReduction, double momentOfInertia) {
  ...
  public MarinersSimMotor(
    ...
    motor =
        new DCMotorSim(
            LinearSystemId.createDCMotorSystem(kV, kA), motorType.withReduction(motorReduction));
   ...
   * @param unitConversion the unit conversion for the mechanism (for example, if the mechanism is
   *     in meters, the unit conversion would be the meters per rotation)
   */
  ...
  protected void setOutput(double output, ControlMode controlMode, double feedForward) {
    switch (controlMode) {
      case Position,
          ProfiledPosition,
          Velocity,
          ProfiledVelocity -> throw new UnsupportedOperationException(
          "can't use motor controller in simulation mode for position or velocity control");
