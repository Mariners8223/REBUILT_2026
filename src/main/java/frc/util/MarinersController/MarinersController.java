...
/**
 * A class to control a motor controller this holds the pid controller, feed forward, and profile of
 * the controller this class is used to interface with the motor controller you can run a pid
 * controller on the roborio or the motor controller you can also add a motion profile to the
 * controller also you can add a feed forward to the controller that is based on the measurement (or
 * static) this class is abstract and is meant to be extended for specific motor controllers
 *
...
public abstract class MarinersController {
  ...
  public enum ControlMode {
    /**
     * the controller is stopped (that means no output is sent to the motor) (depends on the idle
     * mode of the motor)
     */
    Stopped,
    ...
    /**
     * the controller is running in duty cycle control mode the output is between -1 and 1 (where -1
     * is full reverse and 1 is full forward)
     */
    DutyCycle,

    /**
     * the controller is running in voltage control mode the output is in volts the max and min is
     * dependent on the battery voltage
     */
    Voltage,

    /**
     * the controller is running in position control mode the output is in position units (default
     * is rotation)
     */
    Position,

    /**
     * the controller is running in velocity control mode the output is in velocity units (default
     * is rotation per second)
     */
    Velocity,

    /**
     * the controller is running in profiled position control mode that means there is a motion
     * profile set for the controller the output is in position units (default is rotation)
     */
    ProfiledPosition,

    /**
     * the controller is running in profiled velocity control mode that means there is a motion
     * profile set for the controller the output is in velocity units (default is rotation per
     * second)
     */
    ProfiledVelocity;
  ...
  /**
   * the location of the controller where it is running this effects the speed of the thread of the
   * motor and also can effect the amount of can bus traffic
   */
  public enum ControllerLocation {
    /**
     * the controller is running on the roborio that means the pid is calculated on the roborio and
     * the output is sent to the motor controller (more can bus traffic)
     */
    RIO,

    /**
     * the controller is running on the motor controller that means the pid is calculated on the
     * motor controller, and the output is sent to the motor, but also there is a motion profile on
     * the roborio that can be used (less can bus traffic)
     */
    MOTOR
  }
  ...
  /**
   * The lock for the setpoint of the controller this is used to prevent the setpoint from being
   * changed while the controller is running (locks when setpoint, goal, or control mode is being
   * changed or used)
   */
  private final ReentrantLock setpointLock = new ReentrantLock();

  /**
   * The lock for the measurements of the controller this is used to prevent the measurements from
   * being changed while the controller is running (locks when measurements are being updated or
   * being used)
   */
  ...
  /**
   * The inputs of the motor controller (filled by the motor extending this class) (fills the inputs
   * with the controller)
   */
  private final MotorInputs motorInputs = new MotorInputs();

  /**
   * the position wrapping min max if the controller is in position control mode and the position is
   * outside of this range, the controller will wrap the position to be within this range the array
   * should be the minimum value first then the maximum units are the units of the measurements
   */
  private Double[] wrappingMinMax;

  /**
   * the soft limits of the controller, the first value is the minimum value and the second value is
   * the maximum value units are the units of the measurements
   */
  private Double[] softLimits;

  /**
   * The deadband of the motor in voltage if the motor output is less than this value, the motor
  ...
  public void runController() {
    ...
    try {
      ...
      double feedForward =
          arbitraryFeedForward + this.feedForward.apply(measurement) * setpoint.position;
      ...
      if (pidController.atSetpoint()) {
        ...
        double staticForce =
            switch (controlMode) {
              case Position, ProfiledPosition -> Math.signum(setpoint.position - measurement)
                  * motorKs;
  ...
  /** updates the inputs of the controller this will be used for logging */
  public void update() {
  ...
  /**
   * gets the current acceleration of the motor after gear ratio (default is rotations per second
   * squared)
   *
  ...
  /**
   * gets the current setpoint of the controller (if using profiled control mode, this will be the
   * next setpoint (not the goal always)
   *
  ...
  /**
   * if using profiled control mode, this will return the goal of the controller (the end state) if
   * not using profiled control mode, this will return the setpoint of the controller
   *
  ...
  /**
   * sets the motor as a follower to another motor, this will make the motor follow the output of
   * the master motor (this will make the motor ignore any reference set to it) can only be undone
   * by restarting the code
   *
   * @param master the primary motor controller (the one that this motor will follow)
   * @param invert true, if the motor should follow the master in reverse (if the master spins
   *     clockwise, this motor will spin counter-clockwise)
   */
  public void setMotorAsFollower(MarinersController master, boolean invert) {
    if (master.getClass() != this.getClass())
      throw new IllegalArgumentException(
          "cannot set a motor as follower to a different kind of motor");
   ...
   *
   * @param setpoint the setpoint of the controller (needs to be appropriately set for the control
   *     mode)
  ...
  public void setReference(double setpoint, ControlMode controlMode, double arbitraryFeedForward) {
    ...
    try {
      ...
      switch (controlMode) {
        case Voltage -> this.setpoint.position =
            MathUtil.clamp(setpoint, maxMinOutput[1], maxMinOutput[0]);
        case DutyCycle -> this.setpoint.position =
            MathUtil.clamp(setpoint, maxMinOutput[1] / 12, maxMinOutput[0] / 12);
   ...
   *     when using profiled position control mode, this would be the position and velocity of the
   *     controlled value when using profiled velocity control mode, this would be the velocity and
   *     acceleration of the controlled value this will be used when the end state first derivative
   *     is not zero
   * @param controlMode the control mode of the controller (needs to be ProfiledPosition or
   *     ProfiledVelocity)
   */
   ...
   *
   * @param brake true if the motor should brake when idle, false if the motor should coast when
   *     idle
   */
  ...
  /**
   * sets the voltage output of the controller equivalent to {@link #setReference(double,
   * ControlMode)} with ControlMode.Voltage
   *
  ...
  /**
   * sets the duty cycle output of the controller equivalent to {@link #setReference(double,
   * ControlMode)} with ControlMode.DutyCycle
   *
   ...
   * @param currentThreshold the current draw the motor will shut off after drawing (can be lower
   *     than limit)
   */
  public abstract void setCurrentLimits(int currentLimit, int currentThreshold);

  /**
   * Enables position wrapping for the controller. This method is used to wrap the position to be
   * within a specified range. For example, if the minimum is 0 and the maximum is 1, and the
   * position is 1.5, the position will be wrapped to 0.5. This is useful for systems that have a
   * continuous range of motion. (like a swerve steer) DO NOT USE THIS FOR SYSTEMS THAT HAVE A
   * LIMITED RANGE OF MOTION (like an elevator) OR CAN BE DAMAGED BY WRAPPING (like a turret)
   *
   * @param minMax An array containing the minimum and maximum values for position wrapping. The
   *     array should have exactly two elements: the minimum value first, then the maximum value. If
   *     null, position wrapping will be disabled.
  ...
  /**
   * sets the static feed forward of the controller (the voltage needed to overcome static friction)
   *
   ...
   * @param feedForward the function that calculates the feed forward of the controller based on the
   *     measurement
   */
  ...
  /**
   * sets a feedForward (different from the static feed forward) is multiplied by the setpoint
   *
  ...
  /**
   * sends the pid to the smart dashboard that way you can change the pid values on the fly WARNING
   * this will break the feed forward if it is not static (base on the measurement) if using a
   * controller that is on the rio, you can't stop the values from being updated on the board and
   * also feedforward will be updated (both of them until you restart the code)
   */
  ...
  /**
   * stops the pid tuning only does something if started the tuning and using the motor controller
   * on the motor
   */
   ...
   *
   * @param measurements the measurements of the controller these are the position, velocity, and
   *     acceleration of the controlled value
   */
   ...
   *
   * @param profile the profile of the controller (needs to be appropriately set for the control
   *     mode) normally, this would be the max velocity and acceleration of the controlled value,
   ...
   * sets the profile of the controller
   *
   * @param constraints the constraints of the profile (needs to be appropriately set for the
   *     control mode) normally, this would be the max velocity and acceleration of the controlled
   *     value, but if used profiled velocity control, this would be the max acceleration and jerk
   */
   ...
   * sets the profile of the controller
   *
   * @param first_derivative the max value of the first derivative of the controlled value (for
   *     example, if using position control, this would be velocity)
   * @param second_derivative the max value of the second derivative of the controlled value (for
   *     example, if using position control, this would be acceleration)
   */
  public void setProfile(double first_derivative, double second_derivative) {
    profile =
        new TrapezoidProfile(new TrapezoidProfile.Constraints(first_derivative, second_derivative));
  }

  /**
   * sets the deadband of the motor in duty cycle if the motor output is less than this value, the
   * motor will not put out any power
   *
  ...
  /**
   * sets the deadband of the motor in voltage if the motor output is less than this value, the
   * motor will not put out any power motors work in duty cycle, so this will convert the voltage to
   * duty cycle (12 volts is 1)
   *
  ...
  /**
   * is the motor inverted true if counterclockwise is positive, false if clockwise is positive
   *
  ...
  /** resets the motor encoder (only works if the measurements are based on the encoder) */
  public abstract void resetMotorEncoder();

  /**
   * sets the motor encoder position (only works if the measurements are based on the encoder) the
   * value is multiplied by the gear ratio
   *
