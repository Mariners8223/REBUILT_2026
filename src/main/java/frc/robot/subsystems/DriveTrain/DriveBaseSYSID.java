...
public class DriveBaseSYSID {
  ...
  public DriveBaseSYSID(DriveBase driveBase, CommandGenericHID controller) {
    ...
    Supplier<Rotation2d> controllerAngle =
        () -> {
          ...
          if (currentDirection == SysIdRoutine.Direction.kReverse)
  ...
  /**
   * Get the drive motors routine for the given direction the routine will run the drive motors at
   * the given voltage and steer the robot with the controller (field oriented)
   *
   * @param direction the direction to get the routine for
   * @return the command to run the routine
   */
  ...
  /**
   * Get the theta routine for the given direction the routine will run the robot in the theta
   * direction with the given voltage (positive is counter-clockwise)
   *
   * @param direction the direction to get the routine for
   * @return the command to run the routine
   */
  ...
  /**
   * Get the steer motors routine for the given direction the routine will run the steer motors at a
   * quasistatic voltage
   *
   * @param direction the direction to get the routine for
   * @return the command to run the routine
  ...
  /**
   * Get the drive motors routine for the given direction the routine will run the drive motors at a
   * quasistatic voltage and steer the robot with the controller (field oriented)
   *
   * @param direction the direction to get the routine for
   * @return the command to run the routine
   */
  public Command getDriveMotorsRoutineQuasistatic(SysIdRoutine.Direction direction) {
    return driveMotorsRoutine
        .quasistatic(direction)
        .beforeStarting(() -> currentDirection = direction);
  }

  /**
   * Get the theta routine for the given direction the routine will run the robot in the theta
   * direction at a quasistatic voltage (positive is counter-clockwise)
   *
   * @param direction the direction to get the routine for
   * @return the command to run the routine
   */
