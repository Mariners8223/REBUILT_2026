...
/**
 * The DriveBase class represents the drivetrain of the robot. It controls the movement and
 * positioning of the robot using swerve drive.
 */
...
public class DriveBase extends SubsystemBase {
  ...
  /**
   * the kinematics of the swerve drivetrain (how the modules are placed and calculating the
   * kinematics)
   */
  private final SwerveDriveKinematics driveTrainKinematics =
      new SwerveDriveKinematics(DriveBaseConstants.MODULE_TRANSLATIONS);

  /**
   * a variable representing the deltas of the modules (how much they have moved since the last
   * update)
   */
  ...
  /**
   * the pose estimator of the drive base (takes the odometry (which is wheel distance and gyro) and
   * calculates the pose of the robot and also can take vision measurements)
   */
  private final SwerveDrivePoseEstimator poseEstimator =
      new SwerveDrivePoseEstimator(
          driveTrainKinematics, new Rotation2d(), moduleDeltas, new Pose2d());
  ...
  public static class DriveBaseInputs {
    ...
    protected SwerveModuleState[] currentStates =
        new SwerveModuleState[] {
          ...
          new SwerveModuleState(),
          new SwerveModuleState()
        }; // the current states of the modules
  ...
  public DriveBase() {
    ...
    if (RobotBase.isReal()) {
      gyro =
          switch (Constants.ROBOT_TYPE) {
            ...
            case REPLAY -> throw new IllegalArgumentException(
                "Robot cannot be replay if it's real");
          };
      gyro.reset(new Rotation2d());
    } else
      gyro =
          new SimGyroIO(() -> driveTrainKinematics.toTwist2d(moduleDeltas), this::getChassisSpeeds);
    ...
    PathFollowingController pathPlannerPIDController =
  ...
  /**
   * drives the robot made for pathplanner driving
   *
   * @param chassisSpeeds the target chassis speeds of the robot
  ...
  /**
   * drives the robot with the sysID routine not using module optimizations
   *
   * @param states the states to drive the robot in (speeds is voltage) in order of front left,
  ...
  public Command startModuleDriveCalibration() {
    return new InstantCommand(
        ...
        .withName("start module drive calibration")
        .ignoringDisable(true);
  }
  ...
  public Command stopModuleDriveCalibration() {
    return new InstantCommand(
        ...
        .withName("Stop Module Drive Calibration")
        .ignoringDisable(true);
  }
  ...
  public Command startModuleSteerCalibration() {
    return new InstantCommand(
        ...
        .withName("start module steer calibration")
        .ignoringDisable(true);
  }
  ...
  public Command stopModuleSteerCalibration() {
    return new InstantCommand(
        ...
        .withName("Stop Module Steer Calibration")
        .ignoringDisable(true);
  }

  /**
   * sets the speed target of the robot during pathplanner driving (overrides the pathplanner) for
   * example used to go towards a note while in-taking
   *
   * @param xSpeed the x speed of the robot in m/s (field relative) (away from the alliance station
   *     is positive)
   * @param ySpeed the y speed of the robot in m/s (field relative) (left from the alliance station
   *     is positive)
   * @param thetaSpeed the theta speed of the robot in rad/s (left is positive)
   */
  public void overrideXYThetaSpeeds(
      DoubleSupplier xSpeed, DoubleSupplier ySpeed, DoubleSupplier thetaSpeed) {
  ...
  /**
   * sets the speed target of the robot during pathplanner driving (overrides the pathplanner) in
   * field relative speeds for example used to go towards a note while in-taking
   *
   * @param xSpeed the x speed of the robot in m/s (field relative) (away from the alliance station
   *     is positive)
   * @param ySpeed the y speed of the robot in m/s (field relative) (left from the alliance station
   ...
   * @param endVelocity the velocity the robot should be in when it reaches the end of the path in
   *     m/s
  ...
  public Command findPath(Pose2d targetPose, double endVelocity) {
    return AutoBuilder.pathfindToPose(
        targetPose, DriveBaseConstants.PathPlanner.PATH_CONSTRAINTS, endVelocity);
  }
  ...
  public Command pathFindToPathAndFollow(PathPlannerPath targetPath) {
    return AutoBuilder.pathfindThenFollowPath(
        targetPath, DriveBaseConstants.PathPlanner.PATH_CONSTRAINTS);
  }
