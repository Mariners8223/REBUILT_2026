...
public class SwerveModuleIOSIM extends SwerveModuleIO {
  ...
  public SwerveModuleIOSIM(SwerveModule.ModuleName name) {
    ...
    driveMotor =
        new MarinersSimMotor(
            name.name() + " Drive Motor",
            driveMotorModel,
            DRIVE_KV * WHEEL_RADIUS_METERS,
            DRIVE_KA * WHEEL_RADIUS_METERS,
            DRIVE_GEAR_RATIO,
            1 / WHEEL_CIRCUMFERENCE_METERS);
  ...
  public void setDriveMotorReference(double reference, double accelerationFeedForward) {
    driveMotor.setReference(
        reference, MarinersController.ControlMode.Velocity, accelerationFeedForward * DRIVE_KA);
  }
