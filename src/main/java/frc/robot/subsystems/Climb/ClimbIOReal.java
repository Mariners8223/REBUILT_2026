...
public class ClimbIOReal implements ClimbIO {
  ...
  public MarinersTalonFX configureMotor() {
    MarinersTalonFX motor =
        new MarinersTalonFX(
            "Climb motor",
            ClimbConstants.CONTROLLER_LOCATION,
            ClimbConstants.MOTOR_ID,
            ClimbConstants.PID);
    motor
        .getMeasurements()
        .setGearRatio(ClimbConstants.ROTATIONS_TO_METERS * ClimbConstants.GEAR_RATIO);
  ...
  public void Update(ClimbInputs inputs) {
    ...
    inputs.pose =
        new Pose3d(
            ClimbConstants.X_ON_ROBOT,
            ClimbConstants.Y_ON_ROBOT,
            ClimbConstants.Z_OFFSET,
            new Rotation3d());
  }
}
