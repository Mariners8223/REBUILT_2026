...
public class Vision extends SubsystemBase {
  ...
  public Vision(
      VisionConsumer poseConsumer,
      Supplier<Pose2d> referncePoseSupplier,
      Supplier<Boolean> ignoreFunnel) {
    ...
    for (int i = 0; i < numOfCameras; i++) {
      if (Constants.ROBOT_TYPE != RobotType.REPLAY)
        cameras[i] =
            new VisionCamera(constants[i], VisionConstants.FIELD_LAYOUT, referncePoseSupplier);
    ...
    for (VisionCamera camera : cameras) {
      ...
      for (int i = 0; i < camera.inputs.targetIDs.length; i++) {
        Optional<Pose3d> targetPose =
            VisionConstants.FIELD_LAYOUT.getTagPose(camera.inputs.targetIDs[i]);

        targetPose.ifPresent(pose3d -> targetLocations.add(pose3d.getTranslation()));
      }

      Logger.recordOutput(
          "Vision/" + camera.cameraName + "/Target Locations",
          targetLocations.toArray(new Translation3d[0]));
      ...
      for (VisionIO.VisionFrame frame : camera.inputs.visionFrames) {
        ...
        var stdDevs =
            getStdDevs(
                frame.averageTargetDistance(), frame.tagCount(), frame.estimationType(), camera);
      ...
      Logger.recordOutput(
          "Vision/" + camera.cameraName + "/Accepted Poses", acceptedPoses.toArray(new Pose3d[0]));
      Logger.recordOutput(
          "Vision/" + camera.cameraName + "/Rejected Poses", rejectedPoses.toArray(new Pose3d[0]));
    }
  }

  /**
   * If the pose is in a valid location. The pose is valid if it is within the field and the height
   * of the robot is in tolerance
   *
  ...
  private Matrix<N3, N1> getStdDevs(
      ...
      double tagCount,
      VisionIO.EstimationType estimationType,
      VisionCamera camera) {
  ...
  private static class VisionCamera {
    ...
    public VisionCamera(
      ...
      this(
          new VisionIOPhoton(constants, fieldLayout, referencePoseSupplier),
          constants.cameraName,
          constants);
    }
